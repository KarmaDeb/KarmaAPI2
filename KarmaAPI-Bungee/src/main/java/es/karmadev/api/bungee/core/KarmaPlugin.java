package es.karmadev.api.bungee.core;
...
public abstract class KarmaPlugin extends Plugin implements APISource {
    ...
    public KarmaPlugin(final boolean protect, final boolean registerPrincipal) throws NoSuchFieldException, IllegalAccessException, AlreadyRegisteredException {
        ...
        logger = LogManager.getLogger(this).overrideLogFunction((string) -> {
            ProxyServer.getInstance().getConsole().sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', string)));
            return null;
        });
    ...
    public final void onEnable() {
        schedulers.put("async", new BalancedScheduler(100, this, 10, 5));
        enable();
    }
    ...
    public final @NotNull String sourceDescription() {
        ...
        if (ObjectUtils.isNullOrEmpty(rawDescription)) return "KarmaPlugin implementation for " + getDescription().getName();
    ...
    public final @NotNull TaskScheduler scheduler(final String name) {
        return schedulers.computeIfAbsent(name, (s) -> new BalancedScheduler(100, this, 10, 5));
    }
