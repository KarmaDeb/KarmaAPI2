...
import es.karmadev.api.spigot.inventory.helper.func.FunctionalInventory;
import es.karmadev.api.spigot.inventory.helper.func.ItemFunction;
import es.karmadev.api.spigot.inventory.helper.infinity.func.BookAction;
import es.karmadev.api.spigot.inventory.helper.infinity.func.BookItemFunction;
...
public final class InventoryPage implements FunctionalInventory, InventoryHolder, Listener {
    ...
    private final Map<Integer, ItemFunction<InventoryBook>> functions = new ConcurrentHashMap<>();
        ...
        registrar.getServer().getPluginManager().registerEvent(InventoryClickEvent.class, this, EventPriority.HIGHEST, (listener, event) -> {
            ...
            if (click.getClickedInventory() != null) {
                if (click.getClickedInventory().getHolder() == this) {
                    ...
                    ItemFunction<InventoryBook> function = functions.getOrDefault(click.getSlot(), null);
                    if (function != null) {
     ...
     */
    public ItemFunction<InventoryBook> addItem(final ItemStack item) {
        ItemFunction<InventoryBook> function = null;
        for (int slot = 0; slot < 54; slot++) {
            ItemStack stack = inventory.getItem(slot);
            if (stack == null) {
                inventory.setItem(slot, item);
                function = new BookItemFunction(book);
     ...
     * @return the item function
     */
    public ItemFunction<InventoryBook> setItem(final int index, final ItemStack item) {
        ...
        ItemFunction<InventoryBook> function = new BookItemFunction(book);
